...
public sealed class RaycastBulletSystem : ISystem
{
    ...
    private readonly Random         _rng;
    private readonly ISpatialIndex  _spatial;   // shared broad phase (built this frame by BroadPhaseSystem)
    private readonly List<(Entity e, Vector2 from, Vector2 to)> _seg = new();

    public RaycastBulletSystem(EventBus bus, ParticleSystem fx, Random rng, ISpatialIndex spatial)
    { _bus = bus; _fx = fx; _rng = rng; _spatial = spatial; }
    ...
    public void Update(World world, double dt)
    {
        ...
        foreach (var (bullet, from, to) in _seg)
        {
            ...
            if (PhysicsQueries.Raycast(world, _spatial, from, to, hitMask, out var hit))
            {
