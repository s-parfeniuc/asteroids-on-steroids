...
public sealed class ParticleSystem
{
    ...
    private SpriteInstance[] _spriteBuf = System.Array.Empty<SpriteInstance>();

    public void Draw(IRenderer r, Vector2 offset = default)
    {
        // Batched path: build one sprite instance per live particle and submit them all in a single
        // GPU call (thousands of dots → one draw). Falls back to per-circle fills if unsupported.
        if (r is IParticleBatch batch)
        {
            if (_spriteBuf.Length < _count) _spriteBuf = new SpriteInstance[Math.Max(_count, 256)];
            int n = 0;
            for (int i = 0; i < _count; i++)
            {
                ref readonly var p = ref _p[i];
                float t    = p.MaxLife > 0f ? 1f - p.Life / p.MaxLife : 1f;
                float size = p.Size0 + (p.Size1 - p.Size0) * t;
                if (size <= 0f) continue;
                _spriteBuf[n++] = new SpriteInstance(p.Position + offset, size, Color.Lerp(p.Color0, p.Color1, t));
            }
            batch.DrawSprites(_spriteBuf.AsSpan(0, n));
            return;
        }
