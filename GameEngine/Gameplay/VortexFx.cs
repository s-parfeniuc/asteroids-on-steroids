...
public sealed class VortexFx
{
    ...
    public void DrawStreaks(IRenderer r, Vector2 centre, in VortexFxConfig cfg)
    {
        ...
        for (int i = 0; i < _count; i++)
        {
            ...
            byte a = (byte)Clamp255(200f * fade);
            if (a < 4) continue;
            Vector2 tail = m.Pos - m.Vel * cfg.StreakSeconds;
            r.DrawLine(tail, m.Pos, new Color(cr, cg, cb, a), 2f);
        }
        // Bright eye core the streaks spiral into — the fixed landmark.
        r.FillCircle(centre, 6f, new Color(cr, cg, cb, 210));
        r.DrawCircle(centre, 13f, new Color(cr, cg, cb, 120), 2f);
    }

    /// <summary>Swirl-warp the frame around the eye. Call in screen space, after the world is drawn.</summary>
    public void DrawWarp(IPostEffects? post, Camera camera, Vector2 centre, in VortexFxConfig cfg)
    {
        if (post == null || !cfg.Enabled || !cfg.WarpEnabled || cfg.WarpStrength <= 0f) return;

        float rad = cfg.WarpRadius * camera.Zoom;
        Vector2 eye = camera.WorldToScreen(centre);
        Vector2 min = eye - new Vector2(rad), max = eye + new Vector2(rad);
        // Off-screen? nothing to warp.
        if (max.X < 0 || max.Y < 0 || min.X > camera.ScreenWidth || min.Y > camera.ScreenHeight) return;

        float strength = cfg.WarpStrength;
        float rad2 = rad * rad;
        post.Distort(min, max, cfg.WarpGrid, cfg.WarpGrid, p =>
        {
            Vector2 d = p - eye;
            float d2 = d.LengthSquared();
            if (d2 >= rad2 || d2 < 1e-3f) return p;
            float t = 1f - MathF.Sqrt(d2) / rad;      // 1 at eye → 0 at edge
            float ang = strength * t * t;             // twist grows toward the centre
            float c = MathF.Cos(ang), s = MathF.Sin(ang);
            // sample from a point rotated BACKWARD, so content appears rotated forward (a swirl)
            return eye + new Vector2(d.X * c - d.Y * s, d.X * s + d.Y * c);
        });
    }
